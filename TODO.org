...
*** STARTED [#A] Improve knob twisting                          :patcher:web:
    - try out that react knobs library
      - Try moving the onMouseMove and onMouseUp functions to the SVG component itself, and making the SVG area as large as it can be
      - isDragging will need to be moved into the global state
